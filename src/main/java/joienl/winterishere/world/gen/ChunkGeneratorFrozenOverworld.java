...
import static joienl.winterishere.world.biomes.ModBiomes.freezeBiome;
import static joienl.winterishere.world.biomes.ModBiomes.thawBiome;

@SuppressWarnings({"UnusedAssignment", "WeakerAccess", "ConstantConditions"})
public class ChunkGeneratorFrozenOverworld implements IChunkGenerator
{
    ...
    private final IBlockState oceanBlock = Blocks.PACKED_ICE.getDefaultState();
    ...
    public ChunkGeneratorFrozenOverworld(World worldIn, long seed, boolean mapFeaturesEnabledIn, String generatorOptions) {
        woodlandMansionGenerator = new WoodlandMansion(new ChunkGeneratorOverworld(worldIn, seed, mapFeaturesEnabledIn, generatorOptions));{
    ...
    public void setBlocksInChunk(int x, int z, ChunkPrimer primer) {
        ...
        for (int i = 0; i < biomesForGeneration.length; i++) {
            biomesForGeneration[i] = freezeBiome(biomesForGeneration[i]);
        }
    ...
    public Chunk generateChunk(int x, int z) {
        ...
        for (int i = 0; i < biomesForGeneration.length; i++) {
            biomesForGeneration[i] = thawBiome(biomesForGeneration[i]);
        }
    ...
    public void populate(int x, int z) {
        ...
        if (biome != ModBiomes.FROZEN_DESERT && biome != ModBiomes.FROZEN_DESERT_HILLS
                && this.settings.useWaterLakes && !flag && this.rand.nextInt(this.settings.waterLakeChance) == 0
                && biome != ModBiomes.FROZEN_RIVER && biome != ModBiomes.FROZEN_OCEAN && biome != ModBiomes.FROZEN_DEEP_OCEAN
                && biome != ModBiomes.FROZEN_BEACH && biome != ModBiomes.FROZEN_STONE_BEACH) {
            if (TerrainGen.populate(this, this.world, this.rand, x, z, flag, PopulateChunkEvent.Populate.EventType.LAKE)) {
                int i1 = this.rand.nextInt(16) + 8;
                int j1 = this.rand.nextInt(256);
                int k1 = this.rand.nextInt(16) + 8;
                (new WorldGenLakes(Blocks.PACKED_ICE)).generate(this.world, this.rand, blockpos.add(i1, j1, k1));
            }
        }
        ...
        if (!flag && this.rand.nextInt(this.settings.lavaLakeChance / 10) == 0 && this.settings.useLavaLakes
            ...
            if (TerrainGen.populate(this, this.world, this.rand, x, z, flag, PopulateChunkEvent.Populate.EventType.LAVA)) {
                ...
                if (l2 < this.world.getSeaLevel() || this.rand.nextInt(this.settings.lavaLakeChance / 8) == 0) {
                    (new WorldGenLakes(Blocks.OBSIDIAN)).generate(this.world, this.rand, blockpos.add(i2, l2, k3));
                }
            }
        }
            ...
            for (int k2 = 0; k2 < 16; ++k2) {
                for (int j3 = 0; j3 < 16; ++j3) {
                    ...
                    if (this.world.canBlockFreezeWater(blockpos2)) {
                        this.world.setBlockState(blockpos2, Blocks.PACKED_ICE.getDefaultState(), 2);
                    }
    ...
    public boolean isInsideStructure(World worldIn, String structureName, BlockPos pos) {
        ...
        else if ("Frozen Village".equals(structureName) && this.villageGenerator != null) {
