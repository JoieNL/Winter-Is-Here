...
import joienl.winterishere.world.gen.feature.WorldGenGlowStone;
import joienl.winterishere.world.gen.feature.WorldGenHellWater;
import joienl.winterishere.world.gen.structure.MapGenFrozenNetherBridge;
...
public class ChunkGeneratorFrozenHell implements IChunkGenerator
{
    private static final IBlockState AIR = Blocks.AIR.getDefaultState();
    private static final IBlockState NETHERRACK = ModBlocks.FROSTED_NETHERRACK.getDefaultState();
    private static final IBlockState BEDROCK = Blocks.BEDROCK.getDefaultState();
    private static final IBlockState LAVA = Blocks.FLOWING_WATER.getDefaultState();
    ...
    /** Holds the noise used to determine whether soulsand can be generated at a location */
    ...
    /** Determines whether soulsand or gravel can be generated at a location */
    ...
    private final WorldGenGlowStone lightGemGen = new WorldGenGlowStone();
    private final WorldGenGlowStone hellPortalGen = new WorldGenGlowStone();
    private final WorldGenerator quartzGen = new WorldGenMinable(ModBlocks.FROSTED_QUARTZ_ORE.getDefaultState(), 14, BlockMatcher.forBlock(ModBlocks.FROSTED_NETHERRACK));
    private final WorldGenerator magmaGen = new WorldGenMinable(ModBlocks.AQUA_BLOCK.getDefaultState(), 33, BlockMatcher.forBlock(ModBlocks.FROSTED_NETHERRACK));
    private final WorldGenHellWater lavaTrapGen = new WorldGenHellWater(Blocks.FLOWING_WATER, true);
    private final WorldGenHellWater hellSpringGen = new WorldGenHellWater(Blocks.FLOWING_WATER, false);
    @SuppressWarnings("UnusedAssignment")
    private MapGenFrozenNetherBridge genFrozenNetherBridge = new MapGenFrozenNetherBridge();
    @SuppressWarnings("UnusedAssignment")
    ...
    public ChunkGeneratorFrozenHell(World world, boolean generateStructures, long seed)
    {
        this.world = world;
        ...
        InitNoiseGensEvent.ContextHell ctx =
                new InitNoiseGensEvent.ContextHell(lperlinNoise1, lperlinNoise2, perlinNoise1, soulSandGravelNoiseGen, netherrackExculsivityNoiseGen, scaleNoise, depthNoise);
        ...
        this.genFrozenNetherBridge = (MapGenFrozenNetherBridge) TerrainGen.getModdedMapGen(genFrozenNetherBridge, InitMapGenEvent.EventType.NETHER_BRIDGE);
    ...
    private void buildSurfaces(int p_185937_1_, int p_185937_2_, ChunkPrimer primer)
    {
        ...
        this.soulSandNoise = this.soulSandGravelNoiseGen.generateNoiseOctaves(this.soulSandNoise, p_185937_1_ * 16, p_185937_2_ * 16, 0, 16, 16, 1, 0.03125D, 0.03125D, 1.0D);
        this.gravelNoise = this.soulSandGravelNoiseGen.generateNoiseOctaves(this.gravelNoise, p_185937_1_ * 16, 109, p_185937_2_ * 16, 16, 1, 16, 0.03125D, 1.0D, 0.03125D);
        ...
        for (int j = 0; j < 16; ++j)
        {
            for (int k = 0; k < 16; ++k)
            {
                boolean flag = this.soulSandNoise[j + k * 16] + this.rand.nextDouble() * 0.2D > 0.0D;
                ...
                for (int j1 = 127; j1 >= 0; --j1)
                {
                    if (j1 < 127 - this.rand.nextInt(5) && j1 > this.rand.nextInt(5))
                    {
                        ...
                        {
                            if (iblockstate2.getBlock() == ModBlocks.FROSTED_NETHERRACK)
                            {
                                if (i1 == -1)
                                {
                                    if (l <= 0)
    ...
    public Chunk generateChunk(int x, int z)
    {
        ...
        {
            this.genFrozenNetherBridge.generate(this.world, x, z, chunkprimer);
        }
    ...
    public void populate(int x, int z)
    {
        ...
        this.genFrozenNetherBridge.generateStructure(this.world, this.rand, chunkpos);
        ...
        net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new DecorateBiomeEvent.Pre(this.world, this.rand, blockpos));
        ...
        net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new DecorateBiomeEvent.Post(this.world, this.rand, blockpos));
    ...
    public List<Biome.SpawnListEntry> getPossibleCreatures(EnumCreatureType creatureType, BlockPos pos)
    {
        ...
        {
            if (this.genFrozenNetherBridge.isInsideStructure(pos))
            {
                return this.genFrozenNetherBridge.getSpawnList();
            }

            if (this.genFrozenNetherBridge.isPositionInStructure(this.world, pos) && this.world.getBlockState(pos.down()).getBlock() == ModBlocks.FROSTED_NETHER_BRICK)
            {
                return this.genFrozenNetherBridge.getSpawnList();
            }
        }
    ...
    {
        return "Frozen Fortress".equals(structureName) && this.genFrozenNetherBridge != null ? this.genFrozenNetherBridge.getNearestStructurePos(worldIn, position, findUnexplored) : null;
    }

    @Override
    public boolean isInsideStructure(World worldIn, String structureName, BlockPos pos)
    {
        return ("Frozen Fortress".equals(structureName) && this.genFrozenNetherBridge != null) && this.genFrozenNetherBridge.isInsideStructure(pos);
    }
    ...
    {
        this.genFrozenNetherBridge.generate(this.world, x, z, null);
    }
}