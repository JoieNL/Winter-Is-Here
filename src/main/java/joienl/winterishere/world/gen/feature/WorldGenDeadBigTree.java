...
public class WorldGenDeadBigTree extends WorldGenAbstractTree
{
    private Random rand;
    private World world;
    ...
    List<WorldGenDeadBigTree.FoliageCoordinates> foliageCoords;

    public WorldGenDeadBigTree(boolean notify)
    {
        super(notify);
    }
    ...
    void generateLeafNodeList()
    {
        ...
        this.foliageCoords = Lists.<WorldGenDeadBigTree.FoliageCoordinates>newArrayList();
        this.foliageCoords.add(new WorldGenDeadBigTree.FoliageCoordinates(this.basePos.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.checkBlockLine(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.foliageCoords.add(new WorldGenDeadBigTree.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
    ...
    void crossSection(BlockPos pos, float leafSize, IBlockState state)
    {
        ...
        for (int j = -i; j <= i; ++j)
        {
            for (int k = -i; k <= i; ++k)
            {
                if (Math.pow((double)Math.abs(j) + 0.5D, 2.0D) + Math.pow((double)Math.abs(k) + 0.5D, 2.0D) <= (double)(leafSize * leafSize))
                {
                    ...
                    if (state1.getBlock().isAir(state1, world, blockpos) || state1.getBlock().isLeaves(state1, world, blockpos))
                    {
    ...
    void generateLeafNode(BlockPos pos)
    {
        ...
        {
            this.crossSection(pos.up(i), this.leafSize(i), Blocks.AIR.getDefaultState());
        }
    }
    ...
    void generateLeaves()
    {
        for (WorldGenDeadBigTree.FoliageCoordinates worldgenbigtree$foliagecoordinates : this.foliageCoords)
        {
